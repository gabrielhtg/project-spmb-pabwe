    ...
    <section id="jalur-pendaftaran" class="container pt-5">
        <ul class="nav nav-fill nav-pills" id="jalur-pendaftaran-Tab" role="tablist">
            @foreach ($jalurPendaftaran as $key => $e)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $key === 0 ? 'active' : '' }}" id="tab-{{ $e->id }}"
                        data-bs-toggle="tab" data-bs-target="#tab-pane-{{ $e->id }}" type="button" role="tab"
        ...
        <div class="tab-content" id="jenis-pendaftaran-TabContent">
            @foreach ($jalurPendaftaran as $key => $element)
                <div class="tab-pane fade {{ $key === 0 ? 'show active' : '' }}" id="tab-pane-{{ $element->id }}"
                    role="tabpanel" aria-labelledby="{{ $element->id }}-tab" tabindex="0">
                    <div class="d-flex justify-content-center mt-5 ">
                        <div id="carouselExampleIndicators{{ $element->id }}"
                            class="carousel carousel-dark slide border border-1" data-bs-ride="carousel" style="width: 60%">
                            <div class="carousel-inner">
                                @php
                                    $i = 0;
                                @endphp
                            ...
                            <button class="carousel-control-prev" type="button"
                                data-bs-target="#carouselExampleIndicators{{ $element->id }}" data-bs-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                <span class="visually-hidden">Previous</span>
                            </button>
                            <button class="carousel-control-next" type="button"
                                data-bs-target="#carouselExampleIndicators{{ $element->id }}" data-bs-slide="next">
